...
### Milestone 11 — Real Razorpay Test Mode End-to-End Verification: COMPLETE

Goal: prove the existing Razorpay integration (`RazorpayProvider`,
webhook signature verification, correlation, reconciliation) actually
works against the **real, live Razorpay Test Mode API** — not mocks, not
`FakeProvider`, not a hand-rolled simulation — end to end: a real
Payment Link, a real Test Mode payment, a real inbound webhook over the
public internet, real signature verification, and a real
`VERIFYING -> SUCCESS` transition. **Test Mode only. No real money, no
Live Mode credentials, at any point.**

- [x] **Bug found and fixed: Razorpay `reference_id` length limit.**
      The very first real execution attempt (`POST /execute` against
      live Razorpay) failed with a real `HTTP 400 BAD_REQUEST_ERROR:
      "reference_id: the length must be no more than 40."` —
      `RazorpayProvider` had been sending RevGuard's full internal
      idempotency key (`"policy-decision:<uuid>"`, 52 characters)
      directly as Razorpay's `reference_id`, and this had never been
      caught because `RazorpayProvider` had never previously been
      exercised against a real Razorpay account (see Milestone 10's
      "NOT VERIFIED" notes). Fixed in
      `backend/internal/service/razorpay_provider.go` with
      `razorpayReferenceID(idempotencyKey string) string` — SHA-256 of
      the full idempotency key, truncated to 16 bytes (32 hex chars),
      prefixed `rg_`, always exactly 35 characters. RevGuard's own
      `RecoveryAction.IdempotencyKey` is **never** touched, truncated, or
      weakened anywhere else — the transform exists solely at the point
      a value is handed to Razorpay as `reference_id` (shared by both
      `RetryPayment` and `SendPaymentLink` via `createPaymentLink`, one
      transformation, no duplicated logic). Re-verified against the live
      API after the fix: Razorpay returned `200`, and a direct
      `GET /v1/payment_links/{id}` lookup confirmed the stored
      `reference_id` (`rg_8050b9137644497619f9c5e981962e9b`) matched the
      Go transform's output byte-for-byte. 7 new unit tests in
      `razorpay_provider_test.go` (length ≤ 40, determinism, distinct
      inputs -> distinct outputs, safe character set, recognizable
      prefix, original idempotency key never mutated, identical
      transform for both action types).
- [x] **Public webhook exposure via zrok.** This sandbox/dev machine has
      no public HTTPS endpoint, so Razorpay's webhook servers cannot
      reach `localhost:8080` directly. `zrok` (an OpenZiti project,
      Razorpay's own webhook-testing docs recommend it for localhost
      apps) was installed to `/usr/local/bin/zrok`, enabled with a
      user-obtained account token (the token itself was never seen,
      requested, or printed by this work — the user ran `zrok enable
      <token>` themselves in their own terminal; config persists at
      `~/.zrok2`), and run as `zrok share public --headless
      http://localhost:8080`, yielding a real public endpoint
      (`https://<random>.shares.zrok.io`). Verified end-to-end before
      touching Razorpay at all: `GET https://.../health` returned `200`
      from the real backend, and an unsigned `POST
      .../v1/webhooks/razorpay` correctly returned `401` through the
      full internet -> tunnel -> localhost -> RevGuard path.
- [x] **Real webhook registered in Razorpay Dashboard** (Test Mode ->
      Account & Settings -> Webhooks), pointed at the zrok public URL +
      `/v1/webhooks/razorpay`, with exactly three active events —
      `payment_link.paid`, `payment_link.cancelled`,
      `payment_link.expired` — the minimum the current
      `RazorpayWebhookParser` actually recognizes. `payment.captured`/
      `payment.authorized`/`order.paid` were deliberately **not** added
      even though Razorpay does send them for Payment Link payments
      (observed directly — see below) — the current implementation
      doesn't act on them, and per this milestone's explicit scope,
      only the events the implementation actually needs were
      configured, not RevGuard's parser expanded to match whatever
      arrived.
- [x] **Webhook secret handling.** A webhook-secret-exposure incident
      happened mid-session: the user pasted a real secret value into
      chat. It was never reused — flagged immediately, the user was
      walked through regenerating a new secret in the Dashboard, and a
      final secret (self-generated by this work via `openssl rand -hex
      32`, written directly to a local, gitignored, session-scratch env
      file, and never echoed in any response) was independently
      verified (by string comparison only, never printed) to differ
      from the exposed one before proceeding. `RAZORPAY_WEBHOOK_SECRET`
      was never written into any file inside the git repository or
      `.env.example`; it only ever lived in a local runtime env file
      outside the repo.
- [x] **Full real end-to-end run, first attempt — informative partial
      result.** The first real Test Mode payment against a real
      `RazorpayProvider`-created Payment Link succeeded (Razorpay's own
      `GET /v1/payment_links/{id}` confirmed `status: "paid"`,
      `amount_paid: 49950`), but Razorpay delivered only
      `payment.authorized`/`payment.captured` webhooks for that
      delivery — **not** `payment_link.paid` — because the Dashboard's
      active-events selection didn't yet actually include it. RevGuard's
      real behavior on receiving those two events: durably recorded them
      (`provider_webhook_events`, `status=PENDING`), correctly left them
      **unmatched** (`payment.entity.id` is not
      `RecoveryAction.ProviderReference`, which is the Payment Link ID),
      and made **zero** financial-state changes — exactly the documented
      "never guess" behavior, not a bug.
- [x] **Full real end-to-end run, second attempt — genuine success.**
      After confirming `payment_link.paid` was actually enabled and
      creating a fresh case/Payment Link (Razorpay does not retroactively
      redeliver webhooks for already-completed events), the real Test
      Mode payment produced a real `payment_link.paid` webhook delivery,
      correctly signature-verified, correctly correlated
      (`provider_reference = plink_TY3pfHlFdleAny` ->
      `RecoveryAction.ID`), with matching amount/currency
      (`49950`/`INR`, equal to `RevenueAtRisk`), which
      `applyFinancialOutcome` (the same function Milestone 7's
      reconciliation path uses) turned into a durable `RecoveryOutcome`
      (`status=SUCCESS, source=WEBHOOK, provider=razorpay,
      external_reference=plink_TY3pfHlFdleAny`) and a guarded
      `VERIFYING -> SUCCESS` case transition — recorded in a 10-event
      audit trail from `recovery_case.created` through
      `recovery_case.transitioned`/WEBHOOK.
- [x] **Duplicate webhook delivery verified idempotent.** The identical
      real event (same `X-Razorpay-Event-Id`, correctly re-signed) was
      replayed against the already-`SUCCESS` case: response was
      `duplicate: true, financial_outcome_applied: false`; `psql`
      confirmed exactly 1 `provider_webhook_events` row and exactly 1
      `recovery_outcomes` row (recovered amount still `49950`, not
      doubled).
- [x] **Invalid signature verified rejected.** A tampered signature
      against the same real event body was rejected `401`, and `psql`
      confirmed zero new rows anywhere and the existing outcome count
      unchanged at 1.
- [x] **Reconciliation verified safe against an already-resolved case.**
      `POST /reconcile` on the now-`SUCCESS` case correctly refused
      (`422`, "case ... is SUCCESS", not VERIFYING) rather than
      re-processing or conflicting with the webhook-established outcome.
- [x] **Safety scenario verified: policy overrides AI, execution is
      gated.** A payment far above the active policy profile's
      auto-approval ceiling produced a genuine AI recommendation
      (`send_payment_link`, confidence 0.75, positive expected value)
      that the Policy Engine still overrode to `ESCALATE`
      (`AMOUNT_ABOVE_AUTO_LIMIT`) — `POST /execute` against it was
      rejected `422` (`ErrPolicyDecisionNotAllow`) with **zero**
      `recovery_actions` rows and zero Razorpay API calls. Confirms AI
      recommends, policy decides, execution only ever acts on a
      persisted `ALLOW`.
- [x] **Audit-trail gap reviewed (Phase 15), no code change made.**
      `provider_webhook_events` already durably records every
      signature-verified delivery (matched or not); duplicates correctly
      reuse the existing row rather than writing a second one (that's
      what idempotent means, not a gap). The one genuinely log-only case
      is an **invalid-signature rejection** — deliberately not written
      to any table, since `audit_events.recovery_case_id` is `NOT NULL`
      (there is no case to attach an unauthenticated request to) and
      writing DB rows in response to unauthenticated input would itself
      be a minor write-amplification/DoS vector. Judged an acceptable,
      documented trade-off for this milestone, not a gap requiring a
      schema change — worth one sentence in a demo if asked.
- [x] Explicitly NOT implemented / NOT touched this milestone: Live
      Mode (never configured, never tested — this remains permanently
      true until a human deliberately does so outside this codebase's
      test-mode-only posture), any change to `WebhookProcessor`,
      `ReconciliationEngine`, `PolicyEngine`, the state machine, or
      `RazorpayWebhookParser`'s event scope (still exactly the three
      Payment Link lifecycle events), any new dependency beyond the
      `zrok` CLI binary (not a Go/Python/npm dependency — a local dev
      tool, not part of the application), any git operation.

**Verification performed (this milestone, in addition to reusing every
Milestone 0-10 test):**
- `gofmt -l .`, `go build ./...`, `go vet ./...` — clean.
- `TEST_DATABASE_URL=... go test ./...` — all pass (264 tests, including
  the 7 new `razorpay_provider_test.go` tests).
- `TEST_DATABASE_URL=... go test -race -count=1 ./...` — clean, no races.
- `pytest` in `ai-service/` — 36 pass (untouched this milestone).
- `npx tsc --noEmit` and `npm run build` in `frontend/` — clean
  (untouched this milestone; the pre-existing Milestone 11 dashboard —
  `recovery-cases` list/detail, `policies`, `system-health` pages,
  already present as uncommitted work at the start of this session —
  was verified via its real API responses, not modified).
- Real Razorpay Test Mode: authentication (`200` on
  `GET /v1/payment_links`), Payment Link creation (direct API and via
  the real Go `RazorpayProvider`), a real Test Mode card payment, real
  inbound webhook delivery over the public internet through a real
  tunnel, real signature verification (both accept and reject paths),
  real duplicate-delivery idempotency, real reconciliation-safety
  rejection, and a real policy-gated safety scenario — all performed
  against the live `api.razorpay.com`/Dashboard, not simulated.
- Dashboard: verified via direct API responses matching what the
  frontend's TypeScript types expect (`GET /v1/recovery-cases/{id}`
  returned `status: "SUCCESS"`, `recovered_amount_minor_units: 49950`
  after the real webhook). Visual confirmation in an actual browser
  window was **not** performed in this session (no headless browser
  available here — same documented limitation as Milestone 10); the
  user was given the exact case URL to open in their own browser.

**Real Razorpay Test Mode verification status (per this milestone's
explicit reporting requirement):**

| Item | Status |
|---|---|
| Razorpay Test Mode API authentication | **VERIFIED** |
| Razorpay Payment Link creation (direct API) | **VERIFIED** |
| RevGuard Go `RazorpayProvider` (`retry_payment` and `send_payment_link`) | **VERIFIED** |
| `reference_id` 40-character bug | **FOUND AND FIXED**, re-verified live |
| Public webhook URL (zrok tunnel) | **VERIFIED** (`/health` and `/v1/webhooks/razorpay` both reachable) |
| Webhook signature verification (accept + reject paths) | **VERIFIED**, against real Razorpay-style and real Razorpay-delivered payloads |
| Real Test Mode payment -> real webhook -> `SUCCESS` | **VERIFIED** |
| Webhook event idempotency (duplicate delivery) | **VERIFIED** |
| Reconciliation safety against an already-resolved case | **VERIFIED** |
| Policy-gated safety (BLOCK/ESCALATE prevents execution) | **VERIFIED** |
| Live Mode (real money) | **NEVER TESTED** — no Live Mode credentials were ever configured or used |
| Milestone 8-10 synthetic evaluation numbers | **UNCHANGED AND UNRELATED** — those results remain a separate, labeled SYNTHETIC simulation; nothing in this milestone re-ran or altered them |

**Exact local setup procedure (names only, no values — see each
milestone's existing config docs for where these are read):**
1. Set `PAYMENT_PROVIDER=razorpay`, `RAZORPAY_KEY_ID`,
   `RAZORPAY_KEY_SECRET` in the backend's environment (real Test Mode
   credentials from your own Razorpay account — never committed,
   `.env`/`.env.*` are gitignored).
2. Run the backend (`go run ./cmd/server` from `backend/`) and confirm
   `GET /health` locally.
3. Run `zrok enable <your token>` once (interactively, in your own
   terminal — never share this token); this persists to `~/.zrok2`.
4. Run `zrok share public --headless http://localhost:<BACKEND_PORT>`
   to obtain a real public HTTPS URL.
5. In Razorpay Dashboard (Test Mode -> Account & Settings -> Webhooks),
   add a webhook pointed at `https://<that-url>/v1/webhooks/razorpay`
   with exactly `payment_link.paid`, `payment_link.cancelled`,
   `payment_link.expired` as active events, and a **freshly generated**
   secret.
6. Set that same secret as `RAZORPAY_WEBHOOK_SECRET` in the backend's
   environment, and restart the backend so
   `NewConfiguredWebhookVerifier` picks it up (an unset or stale secret
   fails closed — every webhook rejected, never silently unverified).
7. Drive a case to `ALLOW` via `POST /events`, execute it via
   `POST /v1/recovery-cases/{id}/execute`, open the resulting Payment
   Link's `short_url`, and complete a Test Mode payment using Razorpay's
   published test card values (never real card data).

**Known limitations:**
- Live Mode remains completely untested and unconfigured — this is a
  hard boundary this work did not cross.
- The zrok tunnel URL is ephemeral (tied to the running `zrok share`
  process) — it is not a stable production webhook endpoint and was
  used for this verification session only.
- Dashboard visual confirmation in an actual browser was not performed
  in this sandboxed session (no headless browser available); API-level
  verification was used instead, per the same honesty standard applied
  throughout every prior milestone's "Verification performed" sections.
- The audit-trail treatment of invalid-signature rejections (log-only,
  not a durable row) is an intentional, documented trade-off, not an
  oversight — see above.

**Next milestone: Milestone 12.** Not yet scoped. Do not begin
implementation until explicitly instructed.

## Deployment

Deployment preparation work (not an actual deployment — no cloud
account, domain, or Live Mode credentials were touched to produce this
section). Covers the three application services plus PostgreSQL. **No
architecture change**: Redis and Redpanda remain part of the locked
architecture for future milestones; they are simply not required to run
what exists *today* (confirmed via `GET /v1/system-health`, which
reports both `NOT_CONFIGURED` — no Go code path uses either yet).
...
```
   ...
   Browser  ───────────▶ │  Next.js         │
                         ...
                         └────────┬─────────┘
                                  ...
                                  ▼
                         ┌──────────────────┐        ┌───────────────────┐
   Razorpay ───webhook──▶│  Go backend      │───────▶│  Python AI service │
   (Test Mode)           │  (sole authority │  HTTP  │  (recommendations  │
                         │  for state)      │◀───────│  only, no DB)      │
                         └────────┬─────────┘        └───────────────────┘
                                  │
                                  ▼
                         ┌──────────────────┐
...
Minimum production footprint: **4 processes** — `backend`, `ai-service`,
`frontend`, `postgres`. Redis/Redpanda are optional and can be omitted
entirely (see `docker-compose.prod.yml`) without breaking anything that
exists today; add them back only when a future milestone actually reads
or writes to them.

### 2. Required environment variables (names only — see each variable's
existing comment in `.env.example`/`backend/internal/config/config.go`
for what it does; never commit real values anywhere)

**Backend:**
| Variable | Required? | Notes |
|---|---|---|
| `BACKEND_PORT` | optional (default `8080`) | |
| `DATABASE_URL` | optional | if set, overrides all individual `POSTGRES_*` fields — the single-value shape most managed-Postgres/PaaS providers give you |
| `POSTGRES_HOST`/`POSTGRES_PORT`/`POSTGRES_DB`/`POSTGRES_USER`/`POSTGRES_PASSWORD` | required unless `DATABASE_URL` is set | |
| `POSTGRES_SSLMODE` | optional (default `disable`) | set to `require` for a managed Postgres over TLS, unless already encoded in `DATABASE_URL` |
| `AI_SERVICE_URL` | required | must be the AI service's **internal** URL — never expose ai-service publicly |
| `AI_REQUEST_TIMEOUT_SECONDS` | optional (default `20`) | |
| `POLICY_PROFILE` | optional (default `balanced`) | `conservative`/`balanced`/`aggressive` |
| `PAYMENT_PROVIDER` | optional (default `fake`) | set to `razorpay` for real Test Mode execution |
| `RAZORPAY_KEY_ID` | required if `PAYMENT_PROVIDER=razorpay` | **Test Mode key only** — never a Live Mode key |
| `RAZORPAY_KEY_SECRET` | required if `PAYMENT_PROVIDER=razorpay` | |
| `RAZORPAY_BASE_URL` | optional (default `https://api.razorpay.com/v1`) | |
| `RAZORPAY_WEBHOOK_SECRET` | required to accept real webhooks | unset = every webhook rejected (fails closed, never open) |
| `RECONCILER_FAKE_SCENARIO`/`RECONCILER_FAKE_AMOUNT_MINOR_UNITS`/`RECONCILER_FAKE_CURRENCY` | optional | only used when `PAYMENT_PROVIDER=fake` |

**AI service:**
| Variable | Required? | Notes |
|---|---|---|
| `PORT` | optional (default `8000`) | honored by the Docker image's `uvicorn --port ${PORT:-8000}`; pass `--port` explicitly if running uvicorn directly outside Docker |
| `AI_PROVIDER` | optional (default `mock`) | set to `anthropic` for real LLM calls |
...
| Variable | Required? | Notes |
|---|---|---|
| `NEXT_PUBLIC_API_URL` | required at **build time** | the backend's **public** URL (e.g. `https://api.yourdomain.com`) — Next.js inlines `NEXT_PUBLIC_*` into the client bundle during `next build`; setting it only at container runtime has no effect (see `frontend/Dockerfile`) |
| `PORT` | optional (default `3000`) | read natively by `next start` |

**Never echoed/printed by any tool in this codebase or this
documentation:** `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET`,
`RAZORPAY_WEBHOOK_SECRET`, `ANTHROPIC_API_KEY`, `POSTGRES_PASSWORD`,
`DATABASE_URL`.
...
docker build -t revguard-backend ./backend

# AI service
docker build -t revguard-ai-service ./ai-service

# Frontend — NEXT_PUBLIC_API_URL MUST be set before this build, not after
docker build --build-arg NEXT_PUBLIC_API_URL=https://api.yourdomain.com -t revguard-frontend ./frontend
```
...
```
# Backend
./server                      # reads all config from environment variables

# AI service (outside Docker)
uvicorn app.main:app --host 0.0.0.0 --port "${PORT:-8000}"

# Frontend (outside Docker)
npm run build && npm run start   # next start reads $PORT natively
```

Or, for the whole stack via Docker Compose:
```
...
```
cd backend
go run ./cmd/migrate -command up       # applies every migration in backend/migrations, in order
go run ./cmd/migrate -command version  # confirms current schema version
```

- Migrations are golang-migrate `up`/`down` SQL pairs — **additive only**,
  no destructive `DROP`/`TRUNCATE` in any `up` migration across
  Milestones 1–11.
- Uses `Config.PostgresDSN()` — the same `DATABASE_URL`/`POSTGRES_*`
  variables as the server itself, so `cmd/migrate` and `cmd/server`
  never point at different databases by accident.
- Persistent storage: `docker-compose.prod.yml` uses a named volume
  (`postgres-data`) for the self-hosted `postgres` service. For anything
  beyond a single-VM deployment, use a managed Postgres instance instead
  (RDS, Supabase, Neon, Railway Postgres, etc.) via `DATABASE_URL`, which
  gives you real backups/point-in-time-recovery — a Docker volume on a
  single host is not a backup strategy.
- Run migrations **before** starting the backend for the first time on a
  fresh database (see deployment sequence below); the backend does not
  run migrations itself on startup.

### 6. Health checks

| Endpoint | Purpose | Notes |
|---|---|---|
| `GET /health` (backend) | liveness | static `{"status":"ok"}` — always returns 200 once the process is up, does not check dependencies. Use for a container orchestrator's liveness probe. |
| `GET /v1/system-health` (backend) | readiness | real checks: Postgres via `pool.Ping`, AI service via a real HTTP call to its own `/health`. Reports `UP`/`DOWN`/`NOT_CONFIGURED` per component, never a fabricated `UP`. Use for a readiness probe or an uptime dashboard. |
| `GET /health` (ai-service) | liveness | static `{"status":"ok"}` |

Both `docker-compose.yml` and `docker-compose.prod.yml` already wire
`GET /health` as the backend's and ai-service's Docker healthcheck.

### 7. Webhook configuration

The zrok URL used during Milestone 11's real verification
(`https://rscrzm9m2xt1.shares.zrok.io`) is **ephemeral and
session-specific** — it must never be treated as, or configured as, the
production webhook URL. The production webhook is:

```
https://<production-domain>/v1/webhooks/razorpay
```

Steps:
1. Deploy the backend behind a real, stable HTTPS domain (your hosting
   platform's own TLS termination, or a reverse proxy in front of the
   container — this codebase does not terminate TLS itself).
2. In Razorpay Dashboard → (Test Mode, unless/until a deliberate human
   decision to go Live) → Account & Settings → Webhooks, point a webhook
   at the URL above with exactly `payment_link.paid`,
   `payment_link.cancelled`, `payment_link.expired` as active events —
   the minimum `RazorpayWebhookParser` actually recognizes (see
   Milestone 11; do not add more events unless the parser is extended
   first).
3. Generate a **fresh** secret in that dialog (never reuse a secret that
   has ever appeared in a chat log, screen share, or committed file —
   see Milestone 11's exposure incident) and set it as
   `RAZORPAY_WEBHOOK_SECRET` in the backend's production environment.
4. Restart the backend so `NewConfiguredWebhookVerifier` picks up the
   new secret — it fails closed (rejects every webhook) until a
   non-empty secret is configured, so there is no window where webhooks
   are silently accepted unverified.

### 8. Production deployment sequence

1. Provision PostgreSQL (managed service recommended) and set
   `DATABASE_URL` (or the individual `POSTGRES_*` vars +
   `POSTGRES_SSLMODE=require`).
2. Run `go run ./cmd/migrate -command up` against that database.
3. Deploy `ai-service` first (internal-only, no public port), confirm
   `GET /health` returns `200`.
4. Deploy `backend`, pointed at the migrated database and the
   `ai-service`'s internal URL, confirm `GET /health` and
   `GET /v1/system-health` both report healthy.
5. Build and deploy `frontend` with `NEXT_PUBLIC_API_URL` set to the
   backend's real public URL **at build time**.
6. Point a stable HTTPS domain at the backend, then configure the
   Razorpay webhook per section 7 above, and restart the backend once
   `RAZORPAY_WEBHOOK_SECRET` is set.
7. Smoke-test: `POST /events` → `ALLOW` → `POST /execute` → real
   Payment Link → real Test Mode payment → real webhook → `SUCCESS`,
   exactly as verified end-to-end in Milestone 11 — repeat that
   verification once against the real production deployment before
   considering it launch-ready, don't assume local verification
   transfers automatically to a different host/network.

### 9. Rollback procedure

- **Application code**: redeploy the previous container image/binary —
  no database migration is required to roll back a pure code change,
  since the schema is additive and behavior is entirely
  environment-variable-gated (`PAYMENT_PROVIDER=fake` at any time fully
  disables real Razorpay execution without a redeploy).
- **Database migrations**: `go run ./cmd/migrate -command down` rolls
  back exactly one migration (repeat to go back further); every
  migration in this repository has a paired `down` file. If a `down`
  migration fails partway (e.g. blocked by a data-dependent `CHECK`),
  `go run ./cmd/migrate -command force -version <N>` marks the schema
  clean at a known-good version **without** running SQL — the operator
  is responsible for confirming the actual schema matches `<N>` before
  using it (documented in `cmd/migrate/main.go`).
- **Fastest kill switch for a bad Razorpay integration specifically**:
  set `PAYMENT_PROVIDER=fake` and restart the backend — every new
  execution immediately stops calling the real Razorpay API, with zero
  code change and zero migration.
- **Webhook**: disable or delete the webhook in Razorpay Dashboard to
  stop inbound delivery immediately if needed; this does not affect
  already-`SUCCESS`/`FAILED` cases, only future webhook-driven
  resolution (reconciliation via `POST /reconcile` remains available as
  a pull-based alternative).

### 10. Security checklist

- [x] **Webhook signature verification** — HMAC-SHA256 over the exact
      raw body, constant-time compared, fails closed with no configured
      secret. Verified against a real Razorpay Test Mode delivery
      (Milestone 11).
- [x] **Idempotency** — event ingestion, economic evaluation, policy
      decisions, execution, and webhook processing are all idempotent
      (see each milestone's dedicated concurrency tests). Verified with
      a real duplicate webhook delivery in Milestone 11: outcome count
      stayed at 1, recovered amount was not doubled.
- [x] **SQL/database safety** — every query in `internal/repository` is
      parameterized via `pgx`; confirmed via a repo-wide scan for
      string-concatenated/`fmt.Sprintf`-built SQL (none found).
- [x] **Error handling** — every HTTP handler maps internal errors to a
      generic message before responding; raw Postgres/provider errors
      are never leaked to a client (established since Milestone 2,
      re-confirmed by inspection this milestone).
- [x] **Debug logging / stack traces** — `middleware.Recoverer` (chi)
      logs a panic's stack trace to the server's own stderr only, never
      into the HTTP response body; `middleware.Logger` logs
      method/path/status/latency, never headers or bodies (so it never
      logs `X-Razorpay-Signature` or any credential).
- [x] **Production environment flags** — no debug/profiling endpoint is
      wired anywhere (no `net/http/pprof` import in `cmd/server`); the
      Python AI service's FastAPI app is constructed with no `debug=True`.
- [x] **Secret exposure** — this milestone found and fixed a real one:
      `.env.example` (git-tracked) had accumulated real Razorpay
      credentials and the (already-known-exposed) webhook secret in the
      *uncommitted working tree* — never actually pushed to git history,
      but fixed immediately regardless (reset to empty placeholders).
      Confirmed via `git log -- .env.example` that `HEAD` never
      contained real values. A repo-wide grep for the specific exposed
      values found nothing else outside `.claude/settings.local.json`,
      which is covered by the global gitignore, not part of this repo.
- [ ] **Authentication — known, deliberate gap, not fixed this
      milestone.** No endpoint in this API requires authentication.
      `POST /events`, `POST /execute`, `POST /reconcile` are all
      reachable by anyone who can reach the deployed backend. This has
      been true since Milestone 2 and was out of scope for "deployment
      preparation" (adding auth is an architecture decision — which
      scheme, whose identity, what it protects — that needs an explicit
      human decision, not a default this work should invent). **This is
      the single largest gap between "deployable" and "safe to expose
      broadly in a context with real financial stakes."** Contained for
      now only by: (a) Test Mode only, no real money; (b) the Policy
      Engine's own limits (`MaxAutoAmountMinorUnits` etc.) bound the
      blast radius of any single authorized action even if abused; (c)
      every mutation is auditable after the fact. Do not expose this
      deployment on a public URL beyond a controlled demo without
      addressing this first.
- [x] **CORS** — `Access-Control-Allow-Origin: *`, deliberate (see
      `internal/http/cors.go`'s doc comment): every endpoint is either
      read-only or already idempotent, none use cookies or
      browser-managed credentials, so origin-restriction wouldn't add
      meaningful protection here — the real protection boundary is
      "should this endpoint require authentication at all" (see above),
      not CORS. Left unchanged this milestone as instructed
      ("do not modify business logic/architecture unless required").
