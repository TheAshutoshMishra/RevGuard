...
const evaluationDisclaimer = "This evaluation runs entirely against a SYNTHETIC, deterministically generated dataset (docs/architecture/evaluation-engine.md). It has NOT been run against, or validated against, live Razorpay production data, and its results must never be presented as a claim about real-world RevGuard performance."
...
// ComparisonResult is RevGuard measured against one baseline.
type ComparisonResult struct {
	BaselineName string `json:"baseline_name"`

	// IncrementalRecoveredRevenueMinorUnits =
	//   revguard.RevenueRecoveredMinorUnits - baseline.RevenueRecoveredMinorUnits
	IncrementalRecoveredRevenueMinorUnits int64 `json:"incremental_recovered_revenue_minor_units"`

	// IncrementalNetValueMinorUnits =
	//   revguard.NetIncrementalValueMinorUnits - baseline.NetIncrementalValueMinorUnits
	IncrementalNetValueMinorUnits int64 `json:"incremental_net_value_minor_units"`

	// ActionReductionPercent =
	//   (baseline.ActionsTaken - revguard.ActionsTaken) / baseline.ActionsTaken * 100
	// A positive value means RevGuard took fewer actions than the
	...
	// IncrementalRecoveryRate = revguard.RecoveryRate - baseline.RecoveryRate.
...
// TestRunEvaluation_SameSeedIsFullyReproducible).
...
// revGuardStrategyName and the baseline names below must match the
// Name() each strategy returns; RunEvaluation would fail to find the
// corresponding entries in Strategies otherwise (guarded by
// TestRunEvaluation_ComparisonsPresent).
const revGuardStrategyName = "revguard"

// RunEvaluation is Milestone 8's entire evaluation pipeline: generate a
// deterministic synthetic dataset, run every strategy over it, apply the
// shared (strategy-independent) ground truth, aggregate metrics, and
// compare RevGuard against each baseline. It performs no I/O, no
// network calls, and touches no database — every number in the result
// is calculated here from the generated data, never hard-coded.
func RunEvaluation(seed int64, cases int) (*EvaluationResult, error) {
	...
	revguard := strategyResults[revGuardStrategyName]
	comparisons := make(map[string]ComparisonResult, len(strategies)-1)
	for _, strat := range strategies {
		if strat.Name() == revGuardStrategyName {
			continue
		}
		baseline := strategyResults[strat.Name()]
		comparisons["vs_"+strat.Name()] = ComparisonResult{
			BaselineName:                          strat.Name(),
			IncrementalRecoveredRevenueMinorUnits: revguard.RevenueRecoveredMinorUnits - baseline.RevenueRecoveredMinorUnits,
			IncrementalNetValueMinorUnits:         revguard.NetIncrementalValueMinorUnits - baseline.NetIncrementalValueMinorUnits,
			ActionReductionPercent:                actionReductionPercent(baseline.ActionsTaken, revguard.ActionsTaken),
			IncrementalRecoveryRate:               revguard.RecoveryRate - baseline.RecoveryRate,
		}
	}
...
func actionReductionPercent(baselineActionsTaken, revguardActionsTaken int) float64 {
	if baselineActionsTaken == 0 {
		return 0
	}
	return float64(baselineActionsTaken-revguardActionsTaken) / float64(baselineActionsTaken) * 100
}

// FormatResultTable renders the human-readable CLI table described in
// the Milestone 8 brief. Column order is fixed regardless of Go's
// randomized map iteration order, since strategy names are looked up
// explicitly rather than ranged over.
func FormatResultTable(result *EvaluationResult) string {
	...
	fmt.Fprintf(&b, "%-14s %14s %8s %8s %10s %11s %10s %14s %10s %8s %8s\n",
		"Strategy", "Recovered", "Actions", "Blocked", "Escalated", "Unsupport.", "Ambiguous", "NetValue", "Unnecess.", "Rate%", "AvgAtt")
	fmt.Fprintln(&b, strings.Repeat("-", 140))

	order := []string{"fixed_retry", "static_rules", "revguard"}
	for _, name := range order {
		m, ok := result.Strategies[name]
		if !ok {
			continue
		}
		fmt.Fprintf(&b, "%-14s %14d %8d %8d %10d %11d %10d %14d %10d %7.2f%% %8.2f\n",
	...
	b.WriteString("\nComparison (RevGuard vs. baseline):\n")
	for _, name := range []string{"vs_fixed_retry", "vs_static_rules"} {
		c, ok := result.Comparisons[name]
		if !ok {
			continue
		}
		fmt.Fprintf(&b, "  %-16s incremental_recovered_revenue=%d incremental_net_value=%d action_reduction=%.2f%% incremental_recovery_rate=%.4f\n",
			c.BaselineName, c.IncrementalRecoveredRevenueMinorUnits, c.IncrementalNetValueMinorUnits, c.ActionReductionPercent, c.IncrementalRecoveryRate)
	}
...
// report required by Milestone 9. generatedAt and commit are supplied
// by the caller (see cmd/evaluate/main.go) rather than embedded in
// EvaluationResult itself: a wall-clock timestamp or environment-derived
// commit hash would break EvaluationResult's determinism guarantee
// (RunEvaluation must be a pure function of (seed, cases) — see
// TestRunEvaluation_Reproducible). This function is purely
// presentational and has no bearing on the computed metrics.
func FormatMarkdownReport(result *EvaluationResult, generatedAt time.Time, commit string) string {
	...
	b.WriteString("- The ground-truth recoverability model is intentionally independent of RevGuard's\n" +
		...
		"  homework.\n")
	b.WriteString("- RevGuard's simulated execution stage respects Milestone 6's real, current\n" +
		"  limitation: only retry_payment has an execution implementation. A policy ALLOW for any\n" +
		"  other action is authorized but not executed (see \"unsupported_actions\").\n")
	...
	b.WriteString("- **revguard** — the real, unmodified RevGuard pipeline: a deterministic\n" +
		"  AI-diagnosis stand-in (mirrors ai-service's own MockProvider rules) feeds\n" +
		"  HeuristicProbabilityEstimator + GetActionEconomics + the economic formulas\n" +
		"  (Milestone 4) and evaluatePolicyRules + DefaultPolicyConfig (Milestone 5), unmodified.\n" +
		"  Execution is further gated by Milestone 6's real retry_payment-only implementation.\n\n")
	...
	b.WriteString("## Comparison (RevGuard vs. baseline)\n\n")
	b.WriteString("| Baseline | Incremental Recovered Revenue | Incremental Net Value | Action Reduction % | Incremental Recovery Rate |\n")
	b.WriteString("|---|---|---|---|---|\n")
	for _, name := range []string{"vs_fixed_retry", "vs_static_rules"} {
		c, ok := result.Comparisons[name]
		if !ok {
		...
		fmt.Fprintf(&b, "| %s | %d | %d | %.2f%% | %.4f |\n",
			c.BaselineName, c.IncrementalRecoveredRevenueMinorUnits, c.IncrementalNetValueMinorUnits,
	...
	b.WriteString("The comparison table above is computed directly from this run; it is not adjusted\n" +
		"or curated. A positive Incremental Recovered Revenue means RevGuard recovered more than\n" +
		"the named baseline in this run; a negative value means it recovered less. Action Reduction %\n" +
		"reflects how many fewer (or more, if negative) actions RevGuard executed relative to the\n" +
		"baseline. See docs/architecture/evaluation-engine.md and CLAUDE.md's Milestone 9 section for\n" +
		"the specific numbers from the seed used above and an honest discussion of why they came out\n" +
		"the way they did.\n\n")
	...
	b.WriteString("- ExecutionEngine/WebhookProcessor/ReconciliationEngine are not invoked; the ground-truth\n" +
